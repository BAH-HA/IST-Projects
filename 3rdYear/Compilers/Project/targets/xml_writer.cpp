...
{
  os() << std::string(lvl, ' ') << "<stop_node argument='" << node->argument() << "'>" << std::endl;
  closeTag(node, lvl);
}

void til::xml_writer::do_next_node(til::next_node *const node, int lvl)
{
  os() << std::string(lvl, ' ') << "<next_node argument='" << node->argument() << "'>" << std::endl;
  closeTag(node, lvl);
}
...
void til::xml_writer::do_function_call_node(til::function_call_node *const node, int lvl)
{
  ...
  os() << std::string(lvl, ' ') << "<" << node->label() << " name='" << node->func() << "'>" << std::endl;
...
void til::xml_writer::do_print_node(til::print_node *const node, int lvl)
{
  ...
  os() << std::string(lvl, ' ') << "<print_node _new_line='" << bool_to_str(node->new_line()) << "'>" << std::endl;
  node->argument()->accept(this, lvl + 2);
  closeTag(node, lvl);
}
