...
class Board:
    ...
    def adjacent_coordinates(self, row: int, col: int) -> list:
        """Devolve as coordenadas a volta de uma coordenada, incluindo diagonais, 
        caso existam. Verifica casos como fronteiras e cantos."""
        adjacent = []
        if row == 0:
            if col == 0:
                adjacent = [(row, col + 1), (row + 1, col), (row + 1, col + 1)]
            elif col == COLS - 1:
                adjacent = [(row, col - 1), (row + 1, col), (row + 1, col - 1)]
            else:
                adjacent = [(row, col - 1), (row, col + 1), (row + 1, col - 1),
                            (row + 1, col), (row + 1, col + 1)]
        elif row == ROWS - 1:
            if col == 0:
                adjacent = [(row - 1, col), (row - 1, col + 1), (row, col + 1)]
            elif col == COLS - 1:
                adjacent = [(row - 1, col - 1), (row - 1, col), (row, col - 1)]
            else:
    ...
    def surroundPieceWater(self, row: int, col: int):
        """ Coloca agua a volta de uma coordenada"""
        toSurround = self.adjacent_coordinates(row, col)
        for coord in toSurround:
            self.playBoard[coord[0]][coord[1]] = "."
        return

    def surroundHints(self, coordinates: list, boatType: str):
        """ Coloca agua a volta das coordenadas de um barco"""
        for coord in coordinates:
            #adicionar agua a volta
            self.surroundPieceWater(coord[0], coord[1])
            if boatType == "R":
                # adiciona ? a esquerda de R
                self.playBoard[coord[0]][coord[1]-1] = "?"
                # se possivel, adiciona aguas em cima e em baixo, na coluna anterior, de ? (-2 de col)
                if coord[0] > 1:
                    self.playBoard[coord[0]-1][coord[1]-2] = "."
                if coord[0] < ROWS - 2:
                    self.playBoard[coord[0]+1][coord[1]-2] = "."
            elif boatType == "L":
                #adiciona ? a direita de L
                self.playBoard[coord[0]][coord[1]+1] = "?"
                # se possivel, adiciona aguas em cima e em baixo da ? na coluna seguinte a ?
                if coord[0] > 1:
                    self.playBoard[coord[0]-1][coord[1]+2] = "."
                if coord[0] < ROWS - 2:
                    self.playBoard[coord[0]+1][coord[1]+2] = "."
            elif boatType == "T":
                # adiciona ? em baixo de T
                self.playBoard[coord[0]+1][coord[1]] = "?"
                # se possivel, adiciona aguas a esquerda e a direita da ? na linha seguinte a ?
                if coord[1] > 1:
                    self.playBoard[coord[0]+2][coord[1]-1] = "."
                if coord[1] < COLS - 2:
                    self.playBoard[coord[0]+2][coord[1]+1] = "."
            elif boatType == "B":
                # adiciona ? em cima de B
                self.playBoard[coord[0]-1][coord[1]] = "?"
                # se possivel, adiciona aguas a esquerda e a direita da ? na linha anterior a ?
                if coord[1] > 1:
                    self.playBoard[coord[0]-2][coord[1]-1] = "."
                if coord[1] < COLS - 2:
                    self.playBoard[coord[0]-2][coord[1]+1] = "."
    ...
    def parse_instance(): 
        ...
        # coloca agua nas linhas limite 0
        for i in range(ROWS):
            if rowLimits[i] == 0:
                for col in range(COLS):
                    playBoard[i][col] = "."

        # coloca agua nas colunas limite 0
        for i in range(COLS):
            if colLimits[i] == 0:
                for row in range(ROWS):
                    playBoard[row][i] = "."
        ...
        board.surroundHints(submarines, "C")
        board.surroundHints(rightPieces, "R")
        board.surroundHints(leftPieces, "L")
        board.surroundHints(topPieces, "T")
        board.surroundHints(bottomPieces, "B")
